...
export type EstadoRaspberry = {
  ...
  /** No hay tarjeta RPI en el autotanque: no aplica telemetría */
  sin_tarjeta_asignada?: boolean;
};
...
export type TelemetriaAutotanqueConEstado = {
  sin_tarjeta_asignada: boolean;
  autotanque: { id: string; numero: string; placas: string };
  telemetria: TelemetriaVista | null;
  raspberry: EstadoRaspberry;
};
 ...
 */
export async function getTelemetriaPorAutotanqueId(
  autotanqueId: string
): Promise<TelemetriaAutotanqueConEstado | null> {
  const umbral = umbralSegundos();
