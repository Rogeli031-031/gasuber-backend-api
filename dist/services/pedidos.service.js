...
exports.insertPedidoAutotanque = insertPedidoAutotanque;
exports.listPedidosParaConsolaAutotanque = listPedidosParaConsolaAutotanque;
exports.avanzarPedidoEstadoAutotanque = avanzarPedidoEstadoAutotanque;
exports.cancelarPedidoAutotanque = cancelarPedidoAutotanque;
...
async function insertPedidoAutotanque(input) {
    const { rows } = await db_1.db.query(`INSERT INTO pedidos (
       unidad_db_id,
       autotanque_id,
       telefono_origen,
    ...
    const r = rows[0];
    if (!r)
        throw new Error("insertPedidoAutotanque: sin fila");
    return {
        id: String(r.id),
        unidad_db_id: r.unidad_db_id != null ? String(r.unidad_db_id) : null,
        autotanque_id: r.autotanque_id != null
            ? String(r.autotanque_id)
            : null,
        telefono_origen: String(r.telefono_origen),
        cliente_nombre: r.cliente_nombre != null ? String(r.cliente_nombre) : null,
        direccion_texto: String(r.direccion_texto),
...
async function listPedidosParaConsolaAutotanque(autotanque_id, limit = 100) {
    const lim = Math.min(Math.max(1, limit), 200);
...
async function avanzarPedidoEstadoAutotanque(args) {
    const pedidoId = Number(args.pedido_id);
    ...
    try {
       ...
       VALUES ($1, NULL, $2, $3, $4, $5)`, [pedidoId, estadoActual, estadoNuevo, nivelCarb, nivelAlm]);
        await client.query("COMMIT");
        return {
            pedido_id: String(pedidoId),
            estado_anterior: estadoActual,
...
async function cancelarPedidoAutotanque(args) {
    const pedidoId = Number(args.pedido_id);
    const atqId = Number(args.autotanque_id);
    if (!Number.isFinite(pedidoId) || !Number.isFinite(atqId)) {
        throw new Error("pedido_id o autotanque_id inválidos");
    ...
    try {
        await client.query("BEGIN");
        const { rows } = await client.query(`SELECT id::text AS id, estado::text AS estado, autotanque_id::text AS autotanque_id
       FROM pedidos
       WHERE id = $1
       FOR UPDATE`, [pedidoId]);
        const p = rows[0];
       ...
       VALUES ($1, NULL, $2, $3, $4, $5, $6)`, [pedidoId, estadoActual, "cancelado", args.razon_cancelacion, nivelCarb, nivelAlm]);
        await client.query("COMMIT");
        return { pedido_id: String(pedidoId), estado_anterior: estadoActual, estado_nuevo: "cancelado" };
    }
    catch (e) {
