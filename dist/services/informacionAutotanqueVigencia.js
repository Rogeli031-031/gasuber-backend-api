...
exports.celdaRequiereAlerta = celdaRequiereAlerta;
exports.motivoAlerta = motivoAlerta;
const MESES = {
    ene: 0,
    feb: 1,
    mar: 2,
 ...
 */
function ahoraVigencia() {
    return new Date();
}
function esAusente(raw) {
    if (raw == null)
        return true;
...
function finDeMesUtc(year, monthIndex0) {
    return new Date(Date.UTC(year, monthIndex0 + 1, 0, 23, 59, 59, 999));
}
function anioCompletoDosCifras(yy) {
    if (yy >= 100)
        return yy;
 ...
 */
function parseDiaMesAnioCorto(s) {
    const m = s.match(/^(\d{1,2})-([a-z]+)-(\d{2,4})$/i);
    if (!m)
        return null;
    const d = Number(m[1]);
    const mon = MESES[normalizarMesToken(m[2])];
    if (mon === undefined || !Number.isFinite(d) || d < 1 || d > 31)
        return null;
...
function parseComoFinVigencia(s) {
    const t = s.trim();
    if (!t)
        return null;
    return (parseSlashDmy(t) ??
 ...
 */
function celdaRequiereAlerta(vigilancia, raw) {
    if (!vigilancia)
        return false;
...
function motivoAlerta(vigilancia, raw) {
    if (!vigilancia)
        return null;
    if (vigilancia === "presencia") {
