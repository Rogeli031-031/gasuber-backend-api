...
function parsePedidoTextoPlano(raw) {
    ...
    const mLitrosEn = normalized.match(/(?:^|\s)(\d+(?:[.,]\d+)?)\s*litros\s*en\s*(.+)$/i);
    if (mLitrosEn) {
        ...
        direccion = direccion
            ...
            .replace(/^(?:una\\s+pipa\\s+de\\s+|una\\s+pipa\\s+|pipa\\s+de\\s+|pipa\\s+)/i, "")
            .replace(/\s+/g, " ")
            .trim()
            .replace(/[.,;:]+$/g, "");
    ...
    const mLitros = normalized.match(/(?:^|\s)(\d+(?:[.,]\d+)?)\s*litros\s*(?:en\s*)?(.+)$/i);
    if (mLitros) {
